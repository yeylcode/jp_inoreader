...
var base_url_nossl = 'http://jp.inoreader.com/';
...
function init_icon(){
	...
	chrome.browserAction.setIcon({
		path: {
			"19": "icons/icon"+icon+'.png',
			"38": "icons/icon"+icon+"_r.png"
		}
	});
...
if(chrome.runtime && chrome.runtime.sendMessage) {
    chrome.runtime.onMessage.addListener(function (msg) {
        if (msg.option_update) {
            chrome.browserAction.setIcon({
                path: {
                    "19": "icons/icon" + (localStorage['icon'] || 1) + '.png',
                    "38": "icons/icon" + (localStorage['icon'] || 1) + "_r.png"
                }
            });
            get_server_data();
            return;
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function (active) {
                chrome.tabs.query({currentWindow: true}, function (tabs) {
                    ...
                    try{
                    	...
                    	chrome.tabs.create({url: msg.url, active: false, index: idx});
                    }
                });
            });
            return;
